...
def register(app):
    ...
    def update_eda(selected_group, df_data):
        if df_data is None:
        ...
        if selected_group == 'Demográficas':
            return graficos_demograficos(df)
        elif selected_group == 'Temporales':
            return graficos_temporales(df)
        elif selected_group == 'Geográficas':
            return graficos_geograficos(df)
        elif selected_group == 'Clínicas':
            return graficos_clinicos(df)
        else:
...
def graficos_temporales(df):
    ...
    # Gráfico 2: Casos confirmados por año (líneas)
...
def graficos_geograficos(df):
    # Cargar archivo de población por departamento
    deptos = pd.read_csv(r"C:\Users\Hp\VISUAL\entrega_3\poblacion_departamentos_colombia_2018.csv")
    deptos.rename(columns={"Población Censada 2018": "CENSO"}, inplace=True)
    ...
    # Cargar GeoJSON
    with open(r"C:\Users\Hp\VISUAL\entrega_3\Mapa_Depto.geojson", encoding="utf-8") as f:
        geojson = json.load(f)

    # Asignar densidad y casos a cada feature del GeoJSON
    ...
    # Guardar mapa en HTML para luego cargarlo en el iframe
    mapa_path = "mapa_dengue.html"
    m.save(mapa_path)
    ...
    # Leer el HTML guardado del mapa para mostrar en el iframe
    with open(mapa_path, 'r', encoding='utf-8') as f:
        mapa_html = f.read()

    # Retornar el layout con los gráficos y el mapa
    return html.Div([
        html.Div([
            # Gráficos laterales
            html.Div([
...
def graficos_clinicos(df):
    ...
    fig_tip_cas.update_traces(marker=dict(size=6))
    fig_tip_cas.update_layout(
        template="plotly_white",
        hovermode="x unified",
        legend_title="Tipo de Caso",
        colorway=["dodgerblue", "mediumblue"]
    )
